...
//
// Description:		This program allows the user to create encrypted files that
//					can be sent to the target AVR using 'update' program. The
//					program also creates two files for 'bootldr', one which
//					configurates the 'bootldr' according to configuration
//					file parameters, and one for DES decryption keys.
//
...
char *copyrightString =
	"\ncreate v1.0 - Copyright (C) 2003 Atmel Corporation\n\n";
...
char *usageString =
	...
	"  -k filename.ext   DES key table in C-syntax.\n"
...
{	
	...
	try {
		...
		if (parser->getKeyCount() == 0)
			...
			printf("%d-bit %s, cipher block chaining mode\n",
				parser->getKeyCount() * 56,
				(parser->getKeyCount() > 1) ? "Triple-DES" : "Simple-DES");
