...
/* Generate Start Condition */
  ...
  // Inital address tranmission
  PORT_USI &= ~(1<<PIN_USI_SCL);                // Pull SCL LOW
  USIDR     = 0x76 | (FALSE<<TWI_READ_BIT);     // Setup address + R/W bit
  USI_TWI_Master_Transfer( tempUSISR_8bit );    // Send 8 bits on bus.
  ...
  // Write data byte
  PORT_USI &= ~(1<<PIN_USI_SCL);                // Pull SCL LOW.
  USIDR     = 0x06;                             // addr incr & curs shift
  USI_TWI_Master_Transfer( tempUSISR_8bit );    // Send 8 bits on bus.
