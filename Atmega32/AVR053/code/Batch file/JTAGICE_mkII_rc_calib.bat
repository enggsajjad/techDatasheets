...
@SET CPU=ATmega16
@SET TOOL="C:\Program Files\Atmel\AVR Tools\JTAGICEmkII\JTAGICEII.exe"
@SET CAL_FUSES=0x99e4
@SET CUSTOMERCODE="test.hex"


%TOOL% -cUSB -d %CPU% -R -f %CAL_FUSES% -e -pf -if "..\rc_calib.hex"
...
%TOOL% -cUSB -d %CPU% -R -ae0,0 -ve -ie "0xFF_byte.hex"
...
%TOOL% -cUSB -d %CPU% -R -e -pf -vf -if%CUSTOMERCODE% -Z0 -Sf0x01FF
